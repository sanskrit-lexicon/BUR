...
class AB_BUR:
 def __init__(self,line):
  ...
  if m == None:
   print('AB format error',line)
   exit(1)
  self.abid = m.group(1)
  self.disp = m.group(2)
  if self.ab != self.abid:
   print('AB error: "%s" != "%s"' % (self.ab,self.abid))
   print(line)
   exit(1)
...
def get_newlines(lines,dstc):
 newlines = []
 nfound = 0
 ntodo = 0
 for line in lines:
  m = re.search('^(.*?)\t<id>(.*?)</id> <disp>(.*?)</disp>',line)
  old_ab = m.group(1)
  old_id = m.group(2)
  old_disp = m.group(3)
  oldline = m.group(0)
  if old_disp != 'NEW':
   ...
   continue
  # Try to get toolip (new_disp) from STC
  if old_ab in dstc:
   stcrec = dstc[old_ab]
  elif old_ab.lower() in dstc:
   stcrec = dstc[old_ab.lower()]
  else:
   stcrec = None # no help from STC
  if stcrec == None:
   newlines.append(line)
   ntodo = ntodo + 1
   continue
  new_disp = stcrec.disp
  # generate newline
  newline = '%s\t<id>%s</id> <disp>%s</disp>' % (old_ab,old_id,new_disp)
  newlines.append(newline)
  nfound = nfound + 1
 print(nfound,"abbreviations found in Stchoupak")
 print(ntodo,"remaining Burnouf abbreviations without tooltip")
 return newlines
 
if __name__=="__main__":
 filein = sys.argv[1]   # cap_ab.txt
 filein1 = sys.argv[2]  # stcab_input_0.txt
 fileout = sys.argv[3] # burab_input_2.txt

 lines = read_lines(filein)
 lines1 = read_lines(filein1)
 stctips,stctipsdict = init_ab_input(lines1)
 newlines = get_newlines(lines,stctipsdict)
