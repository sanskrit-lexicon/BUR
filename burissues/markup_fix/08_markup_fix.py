"""
Markup fixer + audit for bur.txt (BUR).
Counterpart of PWG#175, PWK#113, and siblings.

BUR-specific notes: 1 leading space in <lang>. 11 non-standard <ab n> (French).

Inputs: ../../../csl-orig/v02/bur/bur.txt
        or argv[1] (any path)
...
if len(sys.argv) >= 3:
    ...
    candidates = [HERE.parent.parent.parent / "csl-orig" / "v02" / "bur" / "bur.txt", HERE / "bur.txt"]
    PW_TXT = next((p for p in candidates if p.exists()), candidates[0])
    OUT_FIX = HERE / "bur_fixed.txt"
...
AUDIT_CHECKS = [
    ...
    ("Empty content tag", re.compile(r"<(ab|s1|lang)\b[^>]*></\1>") if TRIM_TAGS else re.compile(r"(?!x)x")),
...
def main():
    ...
    with OUT_LOG.open("w", encoding="utf-8") as f:
        f.write(f"; markup_fix log for bur.txt\n; nested: {tot_nested}\n; whitespace: {tot_trim}\n; changed: {len(fix_log)}\n;\n")
        for ln,old,new in fix_log: f.write(f"{ln} old {old}\n{ln} new {new}\n")
    with OUT_AUDIT.open("w", encoding="utf-8") as f:
        f.write(f"BUR markup audit\n" + "="*60 + "\n\n")
        f.write(f"Generated by 08_markup_fix.py against bur.txt.\n")
        f.write(f"Paired tags: <ab>, <s1>, <lang>\n")
        f.write(f"Notes: 1 leading space in <lang>. 11 non-standard <ab n> (French).\n\n")
        f.write("AUTO-FIXES: nested <ab> + whitespace in <ab>, <s1>, <lang>\n\n")
